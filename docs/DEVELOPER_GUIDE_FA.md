...
**پنل Providers — دکمه Edit:**

- **Credentials (JSON)**: کلیدهای API Provider را عوض می‌کند (مثلاً `{"apiKey": "..."}` برای SMSIR یا KAVENEGAR). اگر خالی بماند، Credential فعلی دست‌نخورده می‌ماند.
- **Config (JSON)**: تنظیمات قالب Adapter را ویرایش می‌کند؛ برای SMSIR شامل `templateId`، `codeParameter` و `mobileWithCountryCode`. این یعنی تغییر قالب پیامک در پنل sms.ir فقط با به‌روزرسانی `templateId` همین‌جا انجام می‌شود.
- نام، Priority، هزینه هر پیامک، Timeout و کشورهای مجاز هم از همین فرم قابل تغییر است.
